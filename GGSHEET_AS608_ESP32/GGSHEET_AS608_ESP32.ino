...
#define ERA_AUTH_TOKEN "55b7fb90-273f-4910-afdf-a45b1828ef83"
...
const char *GOOGLE_SCRIPT_PATH = "/macros/s/AKfycbxikno_a6IBcvdToXbptJLp_GTT_j4KMz_IjRhmLAVo1BIHmw5GXkpIkW-5gdHZmPM/exec";
#define RELAY_PIN 18
...
void handleAuthenticatedFingerprint(FingerprintAuth auth) {
  ...
  if (auth.id == lastValidatedID && (millis() - lastValidationTime) < 2000) {  // 2 second cooldown
...
void timerEvent() {
  ...
  if (millis() - lastCheck < 500) {  // Check every 0.5 seconds for faster response
    return;
  }
  lastCheck = millis();
...
void setup() {
  ...
  delay(50);                // nhỏ nhưng hữu ích khi cấp nguồn LCD
